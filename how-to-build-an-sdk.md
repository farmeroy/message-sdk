...
- **Auth adapters**: API key from `process.env` in Node vs cookie in browser
- **API design**: how do you type `stream: true` so the return type changes?
  Overloads? Generics? Separate methods? 
...
1. Make the CLI call the Anthropic Messages API. Hardcode everything —
   `fetch`, API key from `process.env`, parse the response. Just get it working.
2. Do the same thing from the browser — a bare HTML file with inline JS is
   fine. Note what breaks and what differs.
3. You now have two implementations with duplicated logic. Identify what's
   shared (request building, response parsing) and what diverges (auth,
   streams). Refactor: shared logic goes into `core`, divergent logic becomes
   platform adapters in `packages/node` and `packages/browser`.
4. Define adapter interfaces in `core`:
   ```typescript
   ...
   ```
5. Implement `NodeHttpAdapter` in `packages/node` — uses Node's native `fetch`,
   reads API key from `process.env.ANTHROPIC_API_KEY`, streams via Node's
   `ReadableStream`.
6. Implement `BrowserHttpAdapter` in `packages/browser` — uses browser `fetch`,
   takes API key as constructor arg (never from env), streams via browser
   `ReadableStream` / SSE with `EventSource` or manual parsing.
7. `core` exports a `createClient(http: HttpAdapter, auth: AuthAdapter)` — it
   builds requests, parses responses, handles the SSE protocol for streaming,
   but never imports platform-specific code.
8. Add streaming (`stream: true`). Parse SSE events, yield tokens via async
   iterators. Start from the CLI, then make it work in the browser.
...
You should be able to explain: why an SDK consumed in the browser  cares about bundle size, what `sideEffects: false` does,
...
- **Bearer tokens**: stateless, sent in `Authorization` header, common in API
  calls from Node/CLI. Your SDK uses this: `x-api-key` header. In a 
  VS Code extension would use a PAT or OAuth token the same way.
...
1. Extend your browser adapter's `AuthAdapter` to accept an optional CSRF
   token and include it as an `X-CSRF-Token` header.
   would need.
